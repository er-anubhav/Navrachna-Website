...
async function seed16StructuredColumns() {
  ...
  const jsonPath = path.join(__dirname, 'full_16_columns_dataset.json')
  const items = JSON.parse(fs.readFileSync(jsonPath, 'utf-8'))

  console.log(`🚀 Seeding all 16 separate columns including Revenue (Rs. Lakhs) for ${items.length} startups...`)
  ...
  console.log(`\n🎉 Successfully synced all 16 separate columns (including Revenue) for ${count} startups!`)
}

seed16StructuredColumns()
