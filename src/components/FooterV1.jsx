...
const DEFAULT_FOOTER = {
  brand: {
    title: "Navrachna Foundation",
    description: "The center provides a range of resources that empower students, faculty & staff to pursue entrepreneurial achievements."
  },
  contact: {
    address: "Navrachna Foundation, Plot no. 46, Knowledge Park 3, Greater Noida",
    phone: "+91 9540527700",
  ...
  bottomBar: {
    copyright: "© Copyright 2026. All Rights Reserved. Navrachna Foundation for Entrepreneurship Development",
    credits: "Crafted and engineered by Orbitron Labs LLP"
  }
}

export function FooterV1({ data: customData }) {
  const { data: dbData } = useCms('footer')
  const data = customData || dbData || DEFAULT_FOOTER

  const brandTitle = data.brand?.title || DEFAULT_FOOTER.brand.title
  const brandDescription = data.brand?.description || DEFAULT_FOOTER.brand.description
  const address = data.contact?.address || DEFAULT_FOOTER.contact.address
  const phone = data.contact?.phone || DEFAULT_FOOTER.contact.phone
  const email = data.contact?.email || DEFAULT_FOOTER.contact.email
  const instagram = data.socials?.instagram || DEFAULT_FOOTER.socials.instagram
  const facebook = data.socials?.facebook || DEFAULT_FOOTER.socials.facebook
  const linkedin = data.socials?.linkedin || DEFAULT_FOOTER.socials.linkedin
  const copyright = data.bottomBar?.copyright || DEFAULT_FOOTER.bottomBar.copyright
  const credits = data.bottomBar?.credits || DEFAULT_FOOTER.bottomBar.credits
      ...
      <div className="relative z-10 mx-auto max-w-[92%] lg:max-w-[1440px] px-4 py-16 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 gap-12 md:grid-cols-2 lg:grid-cols-12">
          ...
          <div className="flex flex-col lg:col-span-3">
            <h3 className="mb-6 text-2xl font-normal font-normal tracking-normal text-white">{brandTitle}</h3>
          ...
          <div className="flex flex-col lg:col-span-4 lg:col-start-9">
            ...
            <div className="mt-8 flex gap-3">
              {instagram && (
                <a href={instagram} target="_blank" rel="noreferrer" className="flex h-10 w-10 items-center justify-center rounded-full bg-black/20 shadow-inner text-[#fbbf24] border border-white/10 backdrop-blur-md hover:-translate-y-1 hover:bg-white/10 transition-all duration-300">
              ...
              {facebook && (
                <a href={facebook} target="_blank" rel="noreferrer" className="flex h-10 w-10 items-center justify-center rounded-full bg-black/20 shadow-inner text-[#fbbf24] border border-white/10 backdrop-blur-md hover:-translate-y-1 hover:bg-white/10 transition-all duration-300">
              ...
              {linkedin && (
                <a href={linkedin} target="_blank" rel="noreferrer" className="flex h-10 w-10 items-center justify-center rounded-full bg-black/20 shadow-inner text-[#fbbf24] border border-white/10 backdrop-blur-md hover:-translate-y-1 hover:bg-white/10 transition-all duration-300">
        ...
        <div className="mt-16 flex flex-col items-center justify-between border-t border-white/10 pt-8 text-xs text-white sm:flex-row">
          ...
          <p className="mt-4 sm:mt-0">
            {credits.includes('Orbitron Labs LLP') ? (
