    ...
    <section className="relative w-full py-20 border-t border-gray-100" style={{ background: bg }}>
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
              ...
              <button
                ...
                className={`px-5 py-2.5 rounded-full text-xs tracking-wide transition-all duration-300 cursor-pointer ${
                  isActive
                    ...
                    : 'bg-slate-100 text-gray-650 hover:bg-slate-200 hover:text-[#013759]'
                }`}
              >
                {f.label || f.title}
              </button>
        ...
        {(() => {
          ...
          const activePremium = premiumDetails[activeFacility % premiumDetails.length]
          const leftSubtitle = spec.identity?.tagline || spec.subtitle || activePremium.subtitle
          const leftTitle = spec.identity?.name || spec.title
          const rawIcon = spec.identity?.icon || spec.icon

          const renderIcon = (icon) => {
            if (!icon) return activePremium.icon
